...
- Q: What is the exact visual treatment for overdue todos ("red text or background" needs concrete definition)? → A: Red text color for todo title + light red/pink background tint on the card
- Q: How does the system handle todos with due dates set to midnight today (when does transition to overdue occur)? → A: Overdue at start of next calendar day (after midnight, when date changes)
- Q: Where exactly should the overdue count badge appear in the UI header? → A: Next to page title "My Todos" in header (e.g., "My Todos (3 overdue)")
- Q: What UI control enables sorting by overdue status? → A: Checkbox with label "Show overdue first" above or below todo list
- Q: Does real-time calculation happen only on user actions or also when the calendar day changes? → A: Recalculate on each component render (React state/props change triggers check)
...
1. **Given** a todo with a due date in the past and status incomplete, **When** the user views the todo list, **Then** the todo displays with overdue styling: red text color for the title and light red/pink background tint on the entire card
...
Users can see at a glance how many overdue todos they have through a count indicator displayed next to the page title "My Todos" in the header (e.g., "My Todos (3 overdue)"), helping them understand their backlog severity without scrolling through the list.
...
**Independent Test**: Can be tested by creating multiple overdue todos and verifying the count displays correctly next to the page title. Delivers value by providing a summary metric of overdue tasks.

**Acceptance Scenarios**:

1. **Given** three incomplete todos with past due dates, **When** the user views the app, **Then** the page title displays "My Todos (3 overdue)"
2. **Given** no todos with past due dates, **When** the user views the app, **Then** the page title displays "My Todos" with no overdue count shown
3. **Given** two incomplete overdue todos and one completed overdue todo, **When** the user views the app, **Then** the page title displays "My Todos (2 overdue)" (only counts incomplete)
4. **Given** user marks an overdue todo as complete, **When** the todo status updates, **Then** the overdue count in the page title decreases by 1

---

### User Story 3 - Sort by Overdue Status (Priority: P3)

Users can optionally sort their todo list to show overdue items at the top by checking a "Show overdue first" checkbox (positioned above or below the todo list), making it easier to focus on past-due tasks without visual scanning.
...
**Independent Test**: Can be tested by checking the "Show overdue first" checkbox and verifying overdue todos appear before non-overdue todos. Delivers value by grouping urgent items together.

**Acceptance Scenarios**:

1. **Given** a mix of overdue and non-overdue todos, **When** the user checks "Show overdue first", **Then** all overdue incomplete todos appear at the top of the list
2. **Given** "Show overdue first" is checked, **When** the user adds a new overdue todo, **Then** it appears in the overdue section at the top
3. **Given** "Show overdue first" is checked, **When** the user marks an overdue todo as complete, **Then** it moves out of the overdue section
4. **Given** "Show overdue first" is checked, **When** the user unchecks the checkbox, **Then** todos return to default sort order (creation date)

---

### Edge Cases

- Real-time updates: Overdue status is recalculated on each component render (when React state/props change). If the app is left open overnight, overdue status will update automatically when any user interaction triggers a re-render (e.g., marking a todo complete, adding a todo). No special midnight timer is required.
- Midnight boundary: A todo due on date X becomes overdue at 12:00:00 AM on date X+1 (start of next calendar day). During the entire day X (even at 11:59 PM), the todo is "due today" not "overdue".
- What if the user's system clock is incorrect? (Rely on browser date/time, accept this limitation)
- How are overdue todos displayed in dark mode vs light mode? (Visual contrast must remain clear in both themes - use theme-appropriate red shades)
...
- **FR-007**: System MUST consider a todo overdue when the current calendar date is after the due date (date-only comparison: todo due on Jan 28 becomes overdue on Jan 29)
...
- Visual styling for overdue items: red text color for todo title + light red/pink background tint on card (using theme's danger/warning colors per UI guidelines)
- Overdue status is calculated client-side on each component render (no special timers needed - React re-renders handle updates)
